...
from torch_int.models.cm_module import bipartite_soft_matching_x_w, merging, do_nothing
...
class CSMModule(nn.Module):
    ...
    def find_outlier_channels(self, x_max, th):
        num_split = torch.ceil(x_max / th)
        num_split = torch.clamp(num_split, min=1.0)
        scaling_factors = 1.0 / num_split
        return num_split, scaling_factors
    ...
    def get_channels_distance_with_weight(self, x, fc_weight):
        ...
        x = x.float()
        scores = bipartite_soft_matching_x_w_scores(
            x, fc_weight
        )
        return scores


    def get_masked_score(self, scores, num_split):
        split_mask = num_split > 1.0
    ...
    def find_outlier_channels_and_store(self, x_max, th):
        with torch.no_grad():
            ...
            num_split, scaling_factors = self.find_outlier_channels(x_max, th)
            scaling_factors = scaling_factors.repeat_interleave(num_split.int())
            if len(outlier_channel_idx) != 0:
                ...
                self.register_buffer("num_split", num_split)
    ...
    def find_threshold(self, x, x_max, attention_mask, position_ids, fcs, function="fc1"):
        ...
        while th >= channelmax_min and cnt < step_num:
            ...
            num_split, scaling_factors = self.find_outlier_channels(x_max, th)
            num_additional_channels = num_split.int().sum().item() - num_channels
            ...
            if num_additional_channels != previous_num_additional_channels:
                # first merge x, do not merge outlier channel
                masked_scores = self.get_masked_score(scores, num_split)
                ...
                # merge x
                merged_x = self.merge_x(x, src_idx, dst_idx, num_additional_channels)
                # merge weight
                merged_weight = self.merge_weight(src_idx, dst_idx, weight)
                # merge num_split
                merged_num_split = self.merge_vector(num_split, src_idx, dst_idx, num_additional_channels)
                # merge scaling factor
                merged_scaling_factors = self.merge_vector(scaling_factors, src_idx, dst_idx, num_additional_channels)

                # split x
                split_x = self.split_x(merged_x, merged_num_split, merged_scaling_factors.repeat_interleave(merged_num_split.int()))
                # split weight
                splited_weight = self.split_weight(merged_num_split, merged_weight)
                # split channel idx
        ...
        plt.show()
        plt.close()
        print("Find threshold {} by minimizing MSE with additional {} channels".format(best_th, best_num_additional_channels))
        return best_th
    ...
    def qkv_function(self, x, weights, attention_mask, position_ids):
        ...
        query_states = query_states.view(bsz, q_len, self.layer.num_heads, self.layer.head_dim).transpose(1, 2)
        key_states = key_states.view(bsz, q_len, self.layer.num_key_value_heads, self.layer.head_dim).transpose(1, 2)
        value_states = value_states.view(bsz, q_len, self.layer.num_key_value_heads, self.layer.head_dim).transpose(1, 2)
        ...
        attn_weights = torch.matmul(query_states, key_states.transpose(2, 3)) / math.sqrt(self.layer.head_dim)
    ...
    def compute_output(self, x, weights, attention_mask, position_ids, function="qkv"):
        if function == "qkv":
            output = self.qkv_function(x, weights, attention_mask.repeat(x.shape[0], 1, 1, 1), position_ids)
        elif function == "fc1":
    ...
    def compute_qoutput(self, x, weights, attention_mask, position_ids, function="qkv"):
        if function == "qkv":
            ...
            output = self.qkv_qfunction(x, weights, attention_mask.repeat(x.shape[0], 1, 1, 1), position_ids)
        elif function == "fc1":
    ...
    def split_x(self, x, num_split, scaling_factors):
        if x.ndim == 2:
            x = x.unsqueeze(0)
        B, N, C = x.shape
        x = x.view(B * N, C)
        x = torch.repeat_interleave(x, num_split.int(), dim=1)
    ...
    def merge_x(self, x, src_idx, dst_idx, num_merged_channels, size=None):
        B, N, C = x.shape
        if size is None:
    ...
    def split_weight(self, num_split, weight):
        split_weight = torch.repeat_interleave(
            weight.data, num_split.int(), dim=1
        )
        return split_weight


    def merge_weight(self, src_idx, dst_idx, weight):
        ...
        merged_weight = weight.data.scatter_reduce(
            dim=-1, index=dst_idx_.expand(cout, r), src=src_weight, reduce="sum"
        )
        merged_weight.data = merged_weight.data.index_select(
            -1, (channel_mask != 0).nonzero().squeeze()
        )
        return merged_weight


    def merge_vector(self, vector, src_idx, dst_idx, num_merged_channels):
        C = vector.shape[0]
        vector = vector.reshape(1, 1, C)
        return self.merge_x(vector, src_idx, dst_idx, num_merged_channels).squeeze()
